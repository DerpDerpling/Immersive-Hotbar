...
public final class ItemAnimationPreview implements ImageRenderer {
    ...
    private static final Identifier HOTBAR_SELECTION_TEXTURE = Identifier.withDefaultNamespace("textures/gui/sprites/hud/hotbar_selection.png");
    ...
    private void renderPickupPopPreview(GuiGraphicsExtractor gfx) {
        gfx.blit(RenderPipelines.GUI_TEXTURED, HOTBAR_TEXTURE, 0, 0, 0, 0, 182, 24, 182, 24);
        ...
        gfx.blit(RenderPipelines.GUI_TEXTURED, HOTBAR_SELECTION_TEXTURE, centerSlotIndex * 20 - 1, -1, 0, 0, 24, 24, 24, 24);

        renderScaledItemIcon(gfx, GRASS_BLOCK_ICON, leftItemX, itemY, stackPlusScale, plusCount);

        if (centerVisible) {
            renderScaledItemIcon(gfx, GRASS_BLOCK_ICON, centerItemX, itemY, newPickupScale, centerCount);
        }

        renderScaledItemIcon(gfx, GRASS_BLOCK_ICON, rightItemX, itemY, stackMinusScale, minusCount);
    }
    ...
    private Identifier iconForMode() {
        return switch (mode) {
            case USE_SHRINK -> IRON_PICKAXE_ICON;
            case SHRINK_OUT_ON_EMPTY, PICKUP_POP, SELECTED_SCALE_COMPARISON -> GRASS_BLOCK_ICON;
        };
    }

    private void renderScaledItemIcon(GuiGraphicsExtractor gfx, Identifier icon, int itemX, int itemY, float renderScale, int count) {
        int cx = itemX + 8;
        int cy = itemY + 8;

        gfx.pose().pushMatrix();
        gfx.pose().translate(cx, cy);
        gfx.pose().scale(renderScale, renderScale);
        gfx.pose().translate(-cx, -cy);
        ...
        if (vanillaBobbingEnabled() && f > 0.0f) {
            float g = 1.0f + f / 5.0f;
            gfx.pose().pushMatrix();
            gfx.pose().translate((float) (itemX + 8), (float) (itemY + 12));
            gfx.pose().scale(1.0f / g, (g + 1.0f) / 2.0f);
            gfx.pose().translate((float) (-(itemX + 8)), (float) (-(itemY + 12)));
            didVanillaPopPose = true;
        }

        gfx.blit(RenderPipelines.GUI_TEXTURED, icon, itemX, itemY, 0, 0, 16, 16, 16, 16);
        ...
        if (count > 1) {
            ...
            int countX = itemX + 19 - 2 - minecraft.font.width(amount);
            int countY = itemY + 9;
        ...
        if (mode == PreviewMode.USE_SHRINK) {
            renderDurabilityBar(gfx, itemX, itemY, renderScale);
        }
    }

    private void renderDurabilityBar(GuiGraphicsExtractor gfx, int itemX, int itemY, float renderScale) {
        // Lightweight visual approximation of the vanilla durability bar for the 26.1 preview.
        int cx = itemX + 8;
        int cy = itemY + 8;
        ...
        gfx.fill(itemX + 2, itemY + 13, itemX + 15, itemY + 15, 0xFF000000);
        gfx.fill(itemX + 2, itemY + 13, itemX + 2 + width, itemY + 14, 0xFF55FF55);

        gfx.pose().popMatrix();
    }

    private void renderSelectedScaleComparison(GuiGraphicsExtractor gfx) {
        gfx.blit(RenderPipelines.GUI_TEXTURED, HOTBAR_TEXTURE, 0, 0, 0, 0, 182, 24, 182, 24);
        ...
        gfx.blit(RenderPipelines.GUI_TEXTURED, HOTBAR_SELECTION_TEXTURE, rightSlotIndex * 20 - 1, -1, 0, 0, 24, 24, 24, 24);

        renderScaledItemIcon(gfx, GRASS_BLOCK_ICON, leftItemX, itemY, scale, 64);
        renderScaledItemIcon(gfx, GRASS_BLOCK_ICON, rightItemX, itemY, secondaryScale, 64);
    }

    private void renderSingleSlot(GuiGraphicsExtractor gfx) {
        gfx.blit(RenderPipelines.GUI_TEXTURED, HOTBAR_TEXTURE, 0, 0, 0, 0, 182, 24, 182, 24);
        gfx.blit(RenderPipelines.GUI_TEXTURED, HOTBAR_SELECTION_TEXTURE, 79, -1, 0, 0, 24, 24, 24, 24);

        if (!itemVisible) return;

        renderScaledItemIcon(gfx, iconForMode(), 83, 3, scale, mode == PreviewMode.USE_SHRINK ? 1 : 64);
    }

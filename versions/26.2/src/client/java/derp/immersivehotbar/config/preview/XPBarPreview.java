...
public final class XPBarPreview implements ImageRenderer {
    ...
    private void renderXpBar(GuiGraphicsExtractor gfx) {
        int x = 0;
        int y = 20;

        gfx.blit(RenderPipelines.GUI_TEXTURED, EXPERIENCE_BAR_BACKGROUND_TEXTURE, x, y, 0, 0, 182, 5, 182, 5);

        int filled = (int) (animatedXpProgress * 183.0f);
        if (filled > 0) {
            gfx.blit(RenderPipelines.GUI_TEXTURED, EXPERIENCE_BAR_PROGRESS_TEXTURE, x, y, 0, 0, filled, 5, 182, 5);
        }
    ...
    private void renderGlow(GuiGraphicsExtractor gfx, int x, int y) {
    ...
    private void renderLevelText(GuiGraphicsExtractor gfx, int barY) {
        ...
        if (pulseScale > 1.0f) {
            ...
            gfx.text(minecraft.font, text, x + 1, y, 0xFF000000, false);
            gfx.text(minecraft.font, text, x - 1, y, 0xFF000000, false);
            gfx.text(minecraft.font, text, x, y + 1, 0xFF000000, false);
            gfx.text(minecraft.font, text, x, y - 1, 0xFF000000, false);
            gfx.text(minecraft.font, text, x, y, 0xFF80FF20, false);
    ...
    private void renderParticles(GuiGraphicsExtractor gfx) {

...
public final class TooltipAnimationRenderer {
    ...
    public static void render(GuiGraphicsExtractor graphics, Minecraft minecraft, TooltipAnimationState state, Component text, int x, int y, int width, int color, TooltipDraw draw) {
    ...
    public static void renderPreview(GuiGraphicsExtractor graphics, Minecraft minecraft, TooltipAnimationState state, Component text, int x, int y) {
        if (state.scale() <= 0.01f || state.fadeSeconds() <= 0.0f) {
            return;
        }
        ...
        graphics.textWithBackdrop(minecraft.font, text, x, y, width, color);

...
public final class ItemAnimationPreview implements ImageRenderer {
    ...
    private void renderSelectedComparison(GuiGraphics gfx) {
        gfx.blitSprite(RenderType::guiTextured, HOTBAR, 0, 0, 182, 24);

        int leftSlot = 3;
        int rightSlot = 5;
        int itemY = 4;

        gfx.blitSprite(RenderType::guiTextured, HOTBAR_SELECTION, rightSlot * 20 - 1, -1, 24, 24);
    ...
    private void renderPickupDemo(GuiGraphics gfx) {
        ...
        gfx.blitSprite(RenderType::guiTextured, HOTBAR_SELECTION, centerSlot * 20 - 1, -1, 24, 24);
    ...
    private void renderSingleSlot(GuiGraphics gfx) {
        gfx.blitSprite(RenderType::guiTextured, HOTBAR, 0, 0, 182, 24);
        gfx.blitSprite(RenderType::guiTextured, HOTBAR_SELECTION, 79, -1, 24, 24);
    ...
    private void renderScaledItem(GuiGraphics gfx, ItemStack stack, int x, int y, float scale) {
        HotbarAnimationRenderer.withItemScale(gfx, x, y, scale, () -> {
            ...
            if (vanillaPose) {
                ...
                gfx.pose().pushPose();
                gfx.pose().translate(x + 8.0f, y + 12.0f, 0.0f);
                gfx.pose().scale(1.0f / bobScale, (bobScale + 1.0f) / 2.0f, 1.0f);
                gfx.pose().translate(-(x + 8.0f), -(y + 12.0f), 0.0f);
            }

            gfx.renderItem(stack, x, y);

            if (vanillaPose) gfx.pose().popPose();

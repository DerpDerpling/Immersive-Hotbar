...
public abstract class OverlayMessageAnimationsMixin {
    ...
    private int immersiveHotbar$animateOverlayMessage(GuiGraphics graphics, Font font, Component text, int x, int y, int width, int color, Operation<Integer> original) {
        if (!animateOverlayMessages) return original.call(graphics, font, text, x, y, width, color);
        ...
        if (immersiveHotbar$forcedShrink || overlayMessageTime <= 18) {
            if (!immersiveHotbar$forcedShrink) {
                immersiveHotbar$naturalShrinkStarted = true;
            }
        ...
        int result = original.call(graphics, font, drawText, drawX, y, drawWidth, color);
